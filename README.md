...
подключённых к Git, и работает в русском и английском интерфейсе. Любую кнопку можно скрыть или показать в разделе
**Окно > Параметры > Быстрые кнопки** (по умолчанию показаны все).
...
- **Слить…**
  - *Что даёт:* слияние выбранной ветки, тега или ref в текущую ветку через штатный механизм EGit.
  - *Как работает:* открывает стандартное окно слияния EGit со штатной Git-иконкой и выполняет обычный merge (может
    создать merge-коммит или сообщить о конфликтах — это поведение самой EGit).
  - *Настройка:* не требуется.
